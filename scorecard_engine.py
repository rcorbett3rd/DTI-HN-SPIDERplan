...
def _score_target_coverage(row: pd.Series, highest_rx: float | None = None) -> tuple[float, str]:
    ...
    # Target D0.03cc high-dose quality. Preferred is <=110%, variable acceptable to <=115%.
    if _finite_float(d003_pct) is not None:
        ...
        notes.append(f"D0.03cc={float(d003_pct):.1f}%Rx scored with 110/115% scale")

    # Highest-dose PTV does not require an _eval structure. Its own V105 is included here.
    if highest_rx is not None and abs(float(assigned_rx) - float(highest_rx)) < 0.05 and _is_ptv(row.get("structure", "")):
        v105 = row.get("V105Rx_%")
